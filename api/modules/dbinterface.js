...
export async function getMySurveys(userid) {
  const sql =
    `SELECT id, usr, name, description, DATE_FORMAT(created, '%d-%M-%Y') AS created FROM surveys WHERE usr=${userid}`;
  const result = await db.query(sql);
  return result;
}
...
export async function getSurveyQuestions(surveyid) {
  const sql =
    `SELECT id, title, description FROM questions WHERE survey=${surveyid}`;
  const result = await db.query(sql);
...
export async function hasUserDone(userid, surveyid) {
  const sql =
    `SELECT response FROM responses WHERE usr=${userid} AND survey=${surveyid}`;
  const result = await db.query(sql);
...
export async function getAverageScore(userid, surveyid) {
  const sql =
    `SELECT AVG(response) 'avg' FROM responses WHERE usr=${userid} AND survey=${surveyid}`;
  console.log(sql);
  const result = await db.query(sql);
  return result[0].avg;
}
