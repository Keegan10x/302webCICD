...
export async function setup(node) {
  ...
  try {
    ...
    node.querySelector('button[name="add"]').addEventListener(
      "click",
      (event) => {
        if (
          ...
          const btnscr =
            "https://cdn.discordapp.com/attachments/814265938832654357/915002023462727750/button.js";
          ...
          removebutton.setAttribute(
            "id",
            "rb" + window.myGlobalVariable.counter.toString(),
          );
          ...
          article.setAttribute(
            "id",
            `${window.myGlobalVariable.counter.toString()}`,
          );
    ...
    node.querySelector('button[name="submit"]').addEventListener(
      "click",
      await send,
    );
...
async function send() {
  if (
    window.myGlobalVariable.questions.length === 0 &&
    window.myGlobalVariable.descriptions.length === 0
  ) {
    ...
    for (const index in window.myGlobalVariable.questions) {
      postQuestions.push({
        title: window.myGlobalVariable.questions[index],
        description: window.myGlobalVariable.descriptions[index],
      });
    }
